...
class Server(object):
    ...
    def receive_models(self,dropout_ratio,required_clients):
        assert (len(self.temp_selected_clients) > 0)
        self.required_clients = required_clients
      # when droupout=0 use temp_selected_clients else use selectde_clients
        if dropout_ratio ==0 :
         active_clients = random.sample(
            self.required_clients, len(self.required_clients))#droup_ration=0.2
        else:
            active_clients = random.sample(
            self.selected_clients, int((1-dropout_ratio) * self.current_num_join_clients))#droup_ration=0.2
    ...
    def aggregate_parameters(self):
        ...
        print("length of uploaded models",len(self.uploaded_models))
